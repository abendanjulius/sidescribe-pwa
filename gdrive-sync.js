...
const GDriveSync = (function () {
  ...
  return {
    ...
    // Silent background check — returns newer cloud data or null
    checkForUpdates: async function () {
      if (_syncing || !_token) return null;
      try {
        const fileId = await findFile();
        ...
        // Cloud is newer by at least 2 seconds
        if (remoteModified > localSyncTime + 2000) {
          const remoteData = await downloadFile(fileId);
          if (remoteData && remoteData.app === 'SideScribe') {
            return remoteData;
        ...
        console.warn('[SideScribe] Background check failed:', err.message);
      }
      return null;
    },
