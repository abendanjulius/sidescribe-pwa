...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function setupGDriveSync() {
    ...
    async function backgroundCheck() {
      ...
      // Don't check if conflict modal is already showing
      if (conflictModal.classList.contains('show')) return;
      try {
        const newerData = await GDriveSync.checkForUpdates();
        if (newerData) {
          console.log('[SideScribe] Cloud data is newer — showing conflict modal');
          // Save current note content before anything
          saveCurrentNoteContent();
          // Show conflict modal (same as manual sync)
          pendingConflictData = newerData;
          showConflictModal();
        }
      } catch (err) {
        console.warn('[SideScribe] Background check error:', err);
      }
    }

    function startAutoCheck() {
      stopAutoCheck();
      autoCheckTimer = setInterval(backgroundCheck, AUTO_CHECK_INTERVAL);
    }
    ...
    // Also check immediately when app becomes visible (user switches back to the PWA)
    document.addEventListener('visibilitychange', function() {
      if (document.visibilityState === 'visible' && GDriveSync.isSignedIn()) {
        // Check right away, then resume normal polling
        backgroundCheck();
        startAutoCheck();
      } else {
        // Save battery when app is hidden
        stopAutoCheck();
      }
    });

    // Stop polling on sign-out, start on sign-in
    var origSignOut = btnSignOut._clickHandler;
